...
export class SeoulCrowdProvider implements CrowdProvider {
  ...
  async getAreaCrowd(areaName: string): Promise<CrowdObservation> {
    ...
    const requestUrl = `${baseUrl}/${encodeURIComponent(key)}/json/citydata_ppltn/1/5/${encodeURIComponent(normalizedArea)}`;
    const sourceUrl = 'https://data.seoul.go.kr/dataList/OA-21285/A/1/datasetView.do';
    ...
    const root = payload.SeoulRtd;
    const code = optionalString(root?.RESULT?.CODE);
    if (!root || (code !== null && code !== 'INFO-000')) {
      throw new BadGatewayException({
        code: 'CROWD_PROVIDER_INVALID_RESPONSE',
        message: 'Seoul Open Data response was invalid',
        details: { providerCode: code, providerMessage: root?.RESULT?.MESSAGE },
      });
    }
    const area = root.citydata_ppltn?.[0];
    const status = area?.LIVE_PPLTN_STTS?.[0];
    if (!area) {
      throw new BadGatewayException({
